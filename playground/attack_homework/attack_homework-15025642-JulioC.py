...
class GeneticAttack:
    ...
    def eval_population(self, population, target_label):
        ...
        # --------------TODO--------------
        score_ranks = np.sort(scores)[::-1] # None  # Sort the scores from largeset to smallest
        best_index = np.where(scores==score_ranks[0])[0][0] # None  # The index for the best scored candidate
        logits = np.exp(scores/self.temperature) # None  # Exponentiate the scores after incorporating temperature
        select_probs = logits/np.sum(logits) # None  # Normalize the logits between 0-1
        # ------------END TODO-------------
        ...
        # lPopulation = sorted(enumerate(population), key= lambda both: select_probs[both[0]], reverse=True)
        # lPopulation =  np.array([both[1] for both in lPopulation])
        # elite = [lPopulation[0]]
        elite = [population[best_index]]
        # elite = []

        # Survived: rest of the top genes that survived, mutated with some probability
        nSurvived = int((1 - self.child_rate)*population.shape[0])
        survIndx = [np.where(scores==score_ranks[i])[0][0] for i in range(1, nSurvived)]
        survived = [self.perturb(population[indx]) if np.random.uniform() < self.mutate_rate else population[indx] for indx in survIndx]  # Survived, and mutate some of them    
        # s0 = lPopulation[1:int((1 - self.child_rate)*lPopulation.shape[0]) ]
        # s0 = population[:int((1 - self.child_rate)*population.shape[0]) ]
        # s1,s2 = s0[:int((1 - self.mutate_rate)*s0.shape[0]) ], s0[int((1 - self.mutate_rate)*s0.shape[0]):]
        # survived = list(s1) + [self.perturb(pic) for pic in s2]  # Survived, and mutate some of them
        ...
        # nChildren = int((self.child_rate)*lPopulation.shape[0])
        # children = [self.crossover(lPopulation[np.random.randint(0, lPopulation.shape[0])],lPopulation[np.random.randint(0, lPopulation.shape[0])]) for t in range(0,nChildren)]
        
        nChildren = int((self.child_rate)*population.shape[0])
        indx = [np.where(scores==score_ranks[i])[0][0] for i in range(0, population.shape[0])]
        # indxProb = np.array([select_probs[i] for i in indx])/np.sum([select_probs[i] for i in indx])
        parentsIndx = [np.random.choice(indx, size=2, replace =False, p = select_probs) for t in range(0, nChildren)]
        children = [self.crossover(population[parentsIndx[i][0]],population[parentsIndx[i][1]]) for i in range(0,nChildren)]
        
        
        # nChildren = int((self.child_rate)*population.shape[0])
        # children = [self.crossover(population[np.random.randint(0, population.shape[0])],population[np.random.randint(0, population.shape[0])]) for t in range(0,nChildren)]
